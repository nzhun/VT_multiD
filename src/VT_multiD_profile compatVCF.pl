...
#	 print "NN ".@newphe."\n";
	...
	# $rawp=$pvalue;
	 $RR=1/$mp*($cs_mat[$PR][$PF]/(0.01+$ctr_mat[$PR][$PF]));
	 $bestF=$F_0+$F_step*$PF;
	 $bestR=$D_0+$D_step*$PR;
...
sub process {
	...
	my $key1=$_[4]; ## REVEL column
	my $key2=$_[5]; ## frequency column
  ...
  while(my $line=<IN>){
       ...
       my @sets=split(/\t+/,$line);
		 my @c=@sets[@includes]; ## genotype loading
 		 ### decode INFO
		 my @alts=split(",",$sets[4]);
		 my %names=%{INFO2map::loadINFO($sets[7])};
 		 for (my $id_alt=0;$id_alt<@alts;$id_alt++){
			 my %sub_names=%{INFO2map::extractINFO(\%names,scalar(@alts),$index)};
			 if(!exists($sub_names{$key1}) ||!exists($sub_names{$key2})){
				 print $key1." or ".$key2." is not find in ".join("\t",@sets[0..3])."\n"; 
				 exit;
			 }
			 my $revel=$sub_names{$key1};
			 my $freq=$sub_names{$key2}; ## get frequency
	    #   my $gene=$sets[5];
			# if($gene =~ /;/){next}
			 foreach my $fkey (keys %col_filter){
				 	if(!exists($sub_names{$fkey}) ){
						print $fkey." is not find in ".join("\t",@sets[0..3])."\n"; 
					 	exit;
				 	}
					if(index($sub_names{$fkey}, $col_filter{$fkey}."|\.")==-1){
		    ...
		    my $alt_id=$id_alt+1;
			 for(my $i=0;$i<@c;$i++){
				  ...
				  if($c[$i] =~ /^0[\/|\|]$alt_id/){
					  $c[$i]=1
				  }elsif($c[$i] =~ /^$alt_id[\/|\|]$alt_id/){
					  $c[$i]=2
				  }elsif($c[$i]!=1 && $c[$i]!=2){next;}
				  my $colTo=0;
...
#	print "tabix $fin $chr -H\n";#open OUT,">$fout";
	...
	for(my $i=0;$i<@sets;$i++){
	  ...
	  int $key1." or $key2 "." were not found in the header\n";
 }
	print "Total samples:".@phe.", cases: $mp\n";
#	print "samples:". join(",",@includes).", cases: $mp\n";
	$mp=$mp/(@phe);
   my @rs=(\@phe,$mp,$key1,$key2,\@includes,\%col_filter);
	return (\@rs);
}
...
my ($phe,$mp,$key1,$key2,$includes,$col_filter)=@{load_info(\%mapInfo)};
my $fout=$mapInfo{"OutName"};
transcript_test($mapInfo{"variantFile"},$mapInfo{"regionFile"},$phe,$mp,$key1,$key2,$includes,$col_filter,$fout);
